...
class PegasusWorkflowManager:
    ...
    def send_logs_and_workflow_to_llm(self, logs, workflow):
        """
    ...
    "messages": [
        {
            ...
            "content": (
                ...
                "as well as the relevance of the problem-solving methods applied.\"\n"
                "  }\n"
                "}\n"
                "Do not include specific corrections to the workflow, changes to YAML files, or detailed implementation steps. "
                "Focus only on describing the nature of the problem and general approaches to solving it.\n\n"
                f"Logs:\n{logs}\n\nWorkflow:\n{workflow}"
            )
        }
    ]
    ...
    def process_workflow(self):
        ...
        try:
            ...
            if logs:
                ...
                llm_response = self.send_logs_and_workflow_to_llm(logs, original_workflow)
                if llm_response:
                    try:
                        #corrected_workflow = self.extract_yaml_from_response(llm_response)
                        #self.save_corrected_workflow(corrected_workflow)
                        print(f"Response Parsed Analyzer")
                        response_analyzer=self.extract_workflow_info(llm_response)
                        # Initialize and process workflow manager
                        workflow_manager_planer = PegasusWorkflowManagerPlanner(
                            workflow_dir=self.workflow_dir,
                            workflow_id= self.workflow_id,
                            api_url="https://api.together.xyz/v1/chat/completions",
                            api_key="9330fcf33a0d19c088c90a6799e1208f30c405627bc12cf27c4d4eaba36dcb96"
                        )
                        workflow_manager_planer.process_workflow(response_analyzer)
